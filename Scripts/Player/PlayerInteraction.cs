...
public sealed class PlayerInteraction : MonoBehaviour
{
    ...
    private enum DetectionType { Raycast, Spherecast, SimpleRaycast };
    ...
    public bool TryFindNearestWorldInteractable(out IInteractable result)
    {
        ...
        if (_detectionType == DetectionType.SimpleRaycast)
        {
            Debug.DrawRay(origin, direction * _interactionRadius, Color.green, 0.5f);

            if (Physics.Raycast(origin, direction, out RaycastHit hit, _interactionRadius, _interactionLayer) &&
                !CheckForObstacle(origin, direction, hit.distance) &&
