...
//    [SerializeField] private SandwichHeatRegistry _sandwichHeatRegistry;
//    [SerializeField] private float _checkBoxSizeMultiple = 0.5f;
...
//    public void OnMicrowaveStartButtonPressed()
//    {
//        Collider[] colliders = Physics.OverlapBox(
//            _cookingZone.transform.TransformPoint(_cookingZone.center),
//            _cookingZone.size * _checkBoxSizeMultiple,
//            _cookingZone.transform.rotation);

//        foreach (var collider in colliders)
//        {
//            if (collider.TryGetComponent(out Sandwich sandwich))
//            {
//                _sandwichHeatRegistry.MarkHeated(sandwich);
//            }
//        }
//    }
...
public sealed class MicrowaveController : MonoBehaviour
{
    ...
    public event System.Action<MicrowaveState> OnStateChanged;
    ...
    private async UniTaskVoid LightingBlinkEffect(CancellationToken token)
    {
        ...
        {
            _lightingEffect.SetActive(true);
            await UniTask.Delay(System.TimeSpan.FromSeconds(_blinkDuration), cancellationToken: token);
            _lightingEffect.SetActive(false);
        }
        catch (OperationCanceledException)
        {
    ...
    private async UniTaskVoid CookingProcess(CancellationToken token)
    {
        try
        {
            await UniTask.Delay(System.TimeSpan.FromSeconds(_cookingDuration), cancellationToken: token);
